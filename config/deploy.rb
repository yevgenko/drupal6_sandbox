...
namespace :deploy do
  ...
  task :symlink, :except => { :no_release => true } do
    asset_folders.each do |asset|
      run "rm -rf #{current_path}/drupal/#{asset}"
      run "ln -nfs #{shared_path}/#{asset} #{current_path}/drupal/#{asset}"
    end
  end
  ...
  task :customize, :except => { :no_release => true } do
    ...
    theme_asset_paths = %w(images css yaml).map { |p| "#{current_path}/sites/all/themes/*/#{p}" }.join(" ")
